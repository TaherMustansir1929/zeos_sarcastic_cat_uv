...
async def ai_handler(bot: Bot, ctx: Context, msg: str):
    """Handle AI assistant requests with rich logging and user feedback."""
    ...
    try:
        ...
        log_info(f"Processing AI request from {ctx.author.name} (ID: {ctx.author.id})")
        
        # Process the request
        with log_loading("Generating AI response..."):
            ai_response = await agent_graph(ctx=ctx, msg=msg, handler="assistant", log=None)
