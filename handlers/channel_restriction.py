...
async def channel_restriction_handler(bot, message):
    ...
    if message.content.startswith(bot.command_prefix):
        ...
        if CHANNEL_ID_BOT_TESTING != 0 and CHANNEL_ID_EXP != 0 and message.channel.id not in [CHANNEL_ID_BOT_TESTING, CHANNEL_ID_EXP]:
            
            allowed_channels = [message.guild.get_channel(CHANNEL_ID_BOT_TESTING), message.guild.get_channel(CHANNEL_ID_EXP)]
            
            if allowed_channels:
                # Send a message directing the user to the correct channel
                await message.channel.send(
                    f"{message.author.mention}, commands can only be used in {allowed_channels[0].mention}"
                )
