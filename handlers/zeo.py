...
async def zeo_handler(bot: Bot, ctx: Context, msg: str):
    ...
    try:
        ...
        log_info(f"Processing Zeo request from {ctx.author.name} (ID: {ctx.author.id})")
        
        # Process the request with a loading indicator
        with log_loading("Generating Zeo's witty response..."):
        ...
        error_msg = f"Error processing Zeo request: {str(e)}"
